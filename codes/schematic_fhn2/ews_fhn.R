...
s<-rep(-0.2,N)
y<-rep(-0.45,N)
dw1<-rnorm(N,0,0.4*sqrt(dt)) # 0.2
dw2<-rnorm(N,0,0.0*sqrt(dt))
for(i in 1:(N-1)){
    s[i+1]<-s[i]+q*(abs(s[i])*(1-s[i])+y[i]+a)*dt+dw1[i]
...
## each event ###############################################################################################
postscript(file="ews_fhn.eps", onefile=FALSE, horizontal=TRUE,encoding="WinAnsi.enc")
...
dw2<-rnorm(N,0,0.0*sqrt(dt))
for(j in 1:(N-1)){
    s[j+1]<-s[j]+q*(abs(s[j])*(1-s[j])+y[j]+a)*dt+dw1[j]
