...
plot(t,x,xlim=c(0,max(T)),type="l",col=4,ylab="",xlab="",bty = "n",axes=F,lwd=1.5)
axis(side=2, cex.axis=2)
axis(side=1, cex.axis=2)
mtext("x", side = 2, line = 3.5, cex=2.5)
mtext("t", side = 1, line = 2.8, cex=2.5)
...
par(oma=c(0,0,1,1))
par(mai = c(0.9, 1, 0.2, 0.2))
plot(y[t>1],x[t>1],type="l",xlim=c(-0.55,-0.2),ylim=c(-0.3,1.1),col=4,ylab="",xlab="",bty = "n",xaxs="i",axes=F,lwd=1.5)
lines(ys,xs,lwd=2.5,col=3)
lines(yf,xs,lwd=2.5,col=6,lty=2)
axis(side=2, cex.axis=2)
axis(side=1, cex.axis=2)
mtext("x", side = 2, line = 3.5, cex=2.5)
mtext("y", side = 1, line = 2.8, cex=2.5)
#mtext("(c)",side=3,adj=0.02,cex=2.5,line=-0.5,font=2,xpd=TRUE)
arrows(-0.36, 1.02, -0.39, 0.98, length = 0.2, angle = 30, code = 2, col = 1, lty = 1, lwd = 1)
