...
GI<-rev(GI)    # detailed   label
GI2<-rev(GI2)  # simplified label
...
## for look #################################################################################################
postscript(file="ews_data/NGRIP_time_series.eps", horizontal=TRUE, encoding="WinAnsi.enc")
par(mfrow=c(2,1))
par(mar=c(0,0,0,0))
par(oma=c(3,0,1,0.5))
...
## each event ###############################################################################################
k<-12       # which proxy and interstadial?
set.seed(k) # set a seed for random values 
if(k==1) postscript(file="ews_data/ews_data_NGRIP_d18O_1.eps", onefile=FALSE, horizontal=TRUE,encoding="WinAnsi.enc")
if(k==2) postscript(file="ews_data/ews_data_NGRIP_d18O_2.eps", onefile=FALSE, horizontal=TRUE,encoding="WinAnsi.enc")
if(k==3) postscript(file="ews_data/ews_data_GRIP_d18O_1.eps", onefile=FALSE, horizontal=TRUE,encoding="WinAnsi.enc")
if(k==4) postscript(file="ews_data/ews_data_GRIP_d18O_2.eps", onefile=FALSE, horizontal=TRUE,encoding="WinAnsi.enc")
if(k==5) postscript(file="ews_data/ews_data_GISP2_d18O_1.eps", onefile=FALSE, horizontal=TRUE,encoding="WinAnsi.enc")
if(k==6) postscript(file="ews_data/ews_data_GISP2_d18O_2.eps", onefile=FALSE, horizontal=TRUE,encoding="WinAnsi.enc")
if(k==7) postscript(file="ews_data/ews_data_NGRIP_ca_1.eps", onefile=FALSE, horizontal=TRUE,encoding="WinAnsi.enc")
if(k==8) postscript(file="ews_data/ews_data_NGRIP_ca_2.eps", onefile=FALSE, horizontal=TRUE,encoding="WinAnsi.enc")
if(k==9) postscript(file="ews_data/ews_data_GRIP_ca_1.eps", onefile=FALSE, horizontal=TRUE,encoding="WinAnsi.enc")
if(k==10) postscript(file="ews_data/ews_data_GRIP_ca_2.eps", onefile=FALSE, horizontal=TRUE,encoding="WinAnsi.enc")
if(k==11) postscript(file="ews_data/ews_data_GISP2_ca_1.eps", onefile=FALSE, horizontal=TRUE,encoding="WinAnsi.enc")
if(k==12) postscript(file="ews_data/ews_data_GISP2_ca_2.eps", onefile=FALSE, horizontal=TRUE,encoding="WinAnsi.enc")

if(k==1)  ii<-c(2, 3, 4, 6, 8, 9)    # remove Eemian (1)
if(k==2)  ii<-c(10,16,19,20,24,33)
if(k==3)  ii<-c(6, 8, 9, 10)
if(k==4)  ii<-c(16,19,20,24,33)
if(k==5)  ii<-c(6, 8, 9, 10)
if(k==6)  ii<-c(16,19,20,24,33)
if(k==7)  ii<-c(4, 6, 8, 9, 10)
if(k==8)  ii<-c(16,19,20,24,33)
if(k==9)  ii<-c(6, 8, 9, 10)
if(k==10) ii<-c(16,19,20,24,33)
if(k==11) ii<-c(6, 8, 9, 10)
if(k==12) ii<-c(16,19,20,24,33)
...
ACF<-matrix(NA,length(y),wd)   # Autocorrelation function
...
sg<-FFTsurrogate(y, n.samples = ns) # surrogate data [# of samples x time]
...
for(j in ix){ 
    vs[,j]<-apply(sg[,(j-wd+1):j],1,var)          # slice & variance for window (j-wd+1):j
    Z<-sg[,(j-wd+1):j]-rowMeans(sg[,(j-wd+1):j])  # slice for window (j-wd+1):j
    acs[,j]<-rowSums(Z[,-1]*Z[,-wd])/rowSums(Z^2) # lag-1 AC for window (j-wd+1):j
...
tm<-tm-rowMeans(tm)
trend1.surrogate<-rowSums(vs*tm)/rowSums(tm^2)    # linear regression coefficient
