...
for(i in 1:(N-1)){
    s[i+1]<-s[i]+50*(abs(s[i])*(1-s[i])+y[i]+p)*dt+dw1[i]
    y[i+1]<-y[i] +0.5*(-s[i]-y[i]+k*(z[i]-y[i]))*dt+dw2[i]
    z[i+1]<-z[i]+0.25*(-s[i]-z[i]+k*(y[i]-z[i]))*dt+dw3[i]
}
#s<-s+rnorm(N,0,0.01*sqrt(dt))

i1<-min(which(s>0.9 & t>20))+100
i2<-min(which(s<0.2 & t>t[i1]))
...
plot(t,s+rnorm(N,0,0.03),type="l",col="gray30")
lines(t,s,col=4)
abline(h=0.3)
lines(t[i1:i3],s[i1:i3],col=4)
...
## each event ###############################################################################################
postscript(file="ews_mmo.eps", onefile=FALSE, horizontal=TRUE,encoding="WinAnsi.enc")
...
j1<-min(which(s>0.9 & t>20))+100
j2<-min(which(s<0.2 & t>t[j1]))
...
#smoothing<-"gaussian"
smoothing<-"loess"
bandwidth<-40 # 0-100% for smoothing 
...
lines(tx,x,col=4,lwd=2)
lines(tx,xs,col=2,lwd=1)
